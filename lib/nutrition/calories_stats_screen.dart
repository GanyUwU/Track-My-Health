...
class CaloriesStatsScreen extends StatefulWidget {
  const CaloriesStatsScreen({super.key, required this.date});
  final DateTime date;
...
class _HeartDetailsScreenState extends State<CaloriesStatsScreen> {
  ...
  Future<void> _fetchCalories() async {
    ...
    if (user != null) {
      ...
      // Assuming the calories are stored under a field named 'calories'
      if (userDoc.exists) {
        setState(() {
          _calories = userDoc['calories'] ?? 0; // Default to 0 if not found
        });
      }
    } else {
                       ...
                       child: StreamBuilder(
                        ...
                        if (snapshot.data!.exists) {
                          if (snapshot.data!
                            ...
                            totalCalories = (snapshot.data!.get('totalCalories') as num).toDouble();
                          }
                          if (snapshot.data!
                              .data()!
                              .containsKey('totalProtein')) {
                            totalProtein = (snapshot.data!.get('totalProtein')as num).toDouble();
                          }
                          if (snapshot.data!.data()!.containsKey('totalFat')) {
                            totalFat = (snapshot.data!.get('totalFat') as num).toDouble();
                          }
                          if (snapshot.data!
                              .data()!
                              .containsKey('totalCarbs')) {
                            totalCarbs = (snapshot.data!.get('totalCarbs') as num).toDouble();
                          }
                            ...
                            Row(
                              ...
                              children: [
                                 Column(
                                  ...
                                  children: [
                                    Text(
                                      //_calorieGoal!.toStringAsFixed(2),
                                      '$_calories',
                                      style: TextStyle(
...
class NutritionRow extends StatelessWidget {
  const NutritionRow(
      {super.key, required this.protein, required this.carbs, required this.fat});
